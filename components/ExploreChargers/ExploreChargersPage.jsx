...
export default function ExploreChargersPage({ state, city, cordinates }) {
	...
	const { filtered_stations, setStations, filterBySocket, filterByVehicle, filterByCity, setMapStation, map_station } =
		useStationStore();
		...
		<div className={styles.warper}>
			...
			<div className='container_wrapper pb-16'>
				...
				<div className='flex justify-between pt-6 h-fit items-center'>
					<p className='text-[#1B1B1B] font-bold text-xl'>
						{open ? map_station?.data?.length : filtered_stations?.data?.length} results
					</p>
								...
								<div className='flex items-center gap-4 select-none'>
									...
									<CgChevronRight
										className={`cursor-pointer ${
											filtered_stations?.pages === page ? 'text-[#BDBDBD]' : 'text-[#1D1E26]'
										}`}
										onClick={() => {
											if (filtered_stations?.pages > page) setPage(prev => prev + 1);
										}}
									/>
									<CgPushChevronRight
										className={`cursor-pointer ${
										...
										onClick={() => setPage(filtered_stations?.pages)}
									/>
								</div>
							</aside>
						</div>
