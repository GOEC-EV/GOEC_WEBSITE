...
export default function Contact({ title }) {
	...
	async function onSubmit(data) {
		...
		const formData = { name: name.trim(), email: data.email, phone: data.phone, city: city.name, gst: state.name };
		try {
			const config = {
				method: 'POST',
				headers: {
			...
			const response = await fetch(
				`https://account.solidperformers.com/leadsapi/data/9156ec6bf325f2143567cf19f06c8133`,
				config
			);
			...
			<main className='container_wrapper flex flex-col items-center  '>
				...
				<form onSubmit={handleSubmit(onSubmit)}>
					<div className={styles.warper2}>
						...
						<p className='text-center text-[#ffffff] text-[13px] leading-normal mt-4'>
